...
void perform_test(struct matrix** input_matricies, int number_of_matricies, int cpu_limit, int mem_limit){
    int i;
    char* buf = (char*) calloc(BUF_SIZE, sizeof(char));
    system("make m");
    sprintf(buf, "./macierz list.txt %d %s %d %d", 5, "common", cpu_limit, mem_limit);  
    system(buf);
...
int main(int argc, char* argv[]){
    ...
    perform_test(input_matricies, number_of_matricies, cpu_limit, mem_limit);
