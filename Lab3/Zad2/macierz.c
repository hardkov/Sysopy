...
int do_the_math(struct matrix** input_matrices, char* com_file, int timeout, char* mode){
    int multiplication_counter, c_index, m_index;
    time_t start_time;
    char* buf;
...
int main(int argc, char* argv[]){
    ...
    char* buf = (char*) calloc(BUF_SIZE, sizeof(char));
    int number_of_workers, timeout, status, i, number_of_matricies;
    pid_t* workers;
    pid_t pid_tmp;
    struct matrix** input_matrices;
    ...
    for(i = 0; i < number_of_workers; i++){
        pid_tmp = fork();
        if(pid_tmp == 0){
            return do_the_math(input_matrices, "com_file.txt", timeout, mode);   // it is executed by a child
        }
        else{
            workers[i] = pid_tmp;
        }
