...
struct block_container* use_create_table(int size){
    struct block_container* (*p_create_table)(int) = (struct block_container* (*) (int))dlsym(handle, "create_table");
    return ((*p_create_table)(size));
}

void use_compare_files(char** files, int size){
    void (*p_compare_files)(char**, int) = (void (*) (char**, int))dlsym(handle, "compare_files");
    (*p_compare_files)(files, size);
}

int use_create_block(char* file, struct block_container* bl_c){
    int (*p_create_block)(char*, struct block_container*) = (int (*) (char*, struct block_container*))dlsym(handle, "create_block");
    return (*p_create_block)(file, bl_c);
}

void use_remove_block(struct block_container* bl_c, int index){
    void (*p_remove_block)(struct block_container*, int) = (void (*) (struct block_container*, int))dlsym(handle, "remove_block");
    (*p_remove_block)(bl_c, index);
}

void use_remove_operation(int bl_index, int index, struct block_container* bl_c){
    void (*p_remove_operation)(int, int, struct block_container*) = (void (*) (int, int, struct block_container*))dlsym(handle, "remove_operation");
    (*p_remove_operation)(bl_index, index, bl_c);
}
...
void perform_test(int size, char* file1, char* file2){
    struct block_container* bl_c = use_create_table(size/2);
    ...
    for(i = 0; i < size/2; i++){
        ...
        use_create_block(tmp_file_name, bl_c);
    }
    ...
    use_create_block("tmp_file0", bl_c);
    use_remove_block(bl_c, 0);
    use_create_block("tmp_file0", bl_c);
    use_remove_block(bl_c, 0);
    use_create_block("tmp_file0", bl_c);
    use_remove_block(bl_c, 0);
...
int main(int argc, char* argv[]){
    ...
    while(n < argc){
        if(strcmp(argv[n], "create_table") == 0){
            bl_c = use_create_table(atoi(argv[n+1]));
            n += 2;
        }
            ...
            use_remove_block(bl_c, atoi(argv[n+1]));
            n += 2;
        }
        else if(strcmp(argv[n], "remove_operation") == 0){
            use_remove_operation(atoi(argv[n+1]), atoi(argv[n+2]), bl_c);
            n += 3;
        }
        else{
            n++;
